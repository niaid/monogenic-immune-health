replace_tbnk_names <- function(vec){
  #tbnk_names_dat <- readr::read_csv("Inputs/Reference/tbnk_names.csv")
  tbnk_names_dat <- read.csv("Inputs/Reference/tbnk_names.csv", stringsAsFactors= FALSE,
                             check.names = FALSE)
...
tbnk_groups <- function(vec, which_col){
  #tbnk_names_dat <- readr::read_csv("Inputs/Reference/tbnk_names.csv")
  tbnk_names_dat <- read.csv("Inputs/Reference/tbnk_names.csv", stringsAsFactors= FALSE,
                             check.names = FALSE)
...
replace_mod_names_single_type <- function(vec, sheet){
  dat <- readxl::read_excel("Inputs/Reference/module_name_map.xlsx", sheet = sheet)
  for(i in ncol(dat)){
...
replace_mod_names_both <- function(vec, proteome_prefix = "protein.", transcriptome_prefix = "gene."){
  library(dplyr)
  dat <- readxl::read_excel("Inputs/Reference/module_name_map.xlsx", sheet = 3)
  for(i in ncol(dat)){
    dat[[i]] <- as.character(dat[[i]])
  }
