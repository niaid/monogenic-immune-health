wilcoxTest = function(gene_set, universe, hits, fix_universe = TRUE, fix_gene_set = TRUE, fix_hits = TRUE, alternative = 'greater') {
  ...
  # If the hits are not already a subset of the universe
  if(fix_hits) {
    # Make them so
    hits = hits[intersect(names(hits), universe)]
  }
  ...
  select = names(hits) %in% gene_set
  
  if(sum(select) == 0) {
    return(list(p.value = NA, effect.size = NA))
  }
  
  pos = hits[select]
  neg =  hits[!select]
  result = wilcox.test(pos, neg, alternative = alternative)
  
  # Extract the pvalue associated with the result
  p.value = result$p.value
  effect.size = result$statistic
...
## 879 (2011).
multiWilcoxTest = function(collectionOfGeneSets, universe, hits, minGeneSetSize = 1,
                           pAdjustMethod = "BH", 
                           fix_gene_sets = T,
                           fix_hits = T,
                           fix_universe = T,
  ...
  dfs = lapply(gene_sets, function(gene_set) {
    ...
    result = wilcoxTest(gene_set, universe, hits, 
                        fix_universe = FALSE, fix_gene_set = FALSE, fix_hits = FALSE, alternative = alternative)
  ...
  # and universe
  colnames(df) = c('Pvalue', 'Effect Size')
  df$`Universe Size` = length(universe)
  df$`Gene Set Size` = gene_set_sizes
  df = df[c('Universe Size', 'Gene Set Size', 'Effect Size', 'Pvalue')]
...
multiWilcoxTests = function(collectionsOfGeneSets, universe, hits, minGeneSetSize = 1,
                            pAdjustMethod = "BH", alternative = 'greater') {
  ...
  results = mapply(function(collectionOfGeneSets, source) {
    ...
    result = multiWilcoxTest(collectionOfGeneSets, universe, hits, minGeneSetSize, pAdjustMethod, alternative = alternative)
